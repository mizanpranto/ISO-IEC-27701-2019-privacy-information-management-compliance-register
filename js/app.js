...
// Privacy control "areas" shared by both Annex A (controller) and Annex B (processor) —
// derived generically from the section label's descriptive suffix, so the same four
// areas aggregate rows from both annexes for the radar chart and role comparison.
const AREA_SHORT_LABEL = {
  "Conditions for collection and processing": "Collection & processing",
  "Obligations to PII principals": "Principal obligations",
  "Privacy by design and privacy by default": "Privacy by design",
  "PII sharing, transfer, and disclosure": "Sharing & transfer",
};
...
function renderDashboard() {
  ...
  // Radar: 4 privacy control areas, combining Annex A (controller) + Annex B (processor)
  const privacyRows = rows.filter(r => r.category === "Annex A Controls" || r.category === "Annex B Controls");
  const areaNames = [...new Set(privacyRows.map(r => themeForSection(r.section)).filter(Boolean))];
  const areaRows = name => privacyRows.filter(r => themeForSection(r.section) === name);
  const radarPoints = areaNames.map(name => {
    const wc = weightedCompliance(areaRows(name));
    return { label: shortAreaLabel(name), value: wc === null ? 0 : wc };
  });
  ...
  // Bars: mandatory (PIMS) clauses by sub-clause group
  renderBars("clauseBars", groupBy(rows.filter(r => r.category === "Mandatory Clauses"), r => r.section));
  // Bars: Controller (Annex A) vs Processor (Annex B) compliance
  renderBars("themeBars", groupBy(privacyRows, r => r.category === "Annex A Controls" ? "Controller (Annex A)" : "Processor (Annex B)"));
...
const VIEW_TITLES = {
  ...
  clauses: "PIMS Clauses",
  annex: "Annex A · Controller",
  annexb: "Annex B · Processor",
...
function goToView(view) {
  ...
  if (view === "annexb") renderSectionDetail("annexBDetail", "Annex B Controls");
